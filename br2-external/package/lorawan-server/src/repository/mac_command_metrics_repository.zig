...
pub const Repository = struct {
    ...
    pub fn insertObservation(self: Repository, command_tag: []const u8, outcome: []const u8, latency_ns: u64) !void {
        self.db.mutex.lock();
        defer self.db.mutex.unlock();

        const sql =
            "INSERT INTO mac_command_metrics(command_tag, outcome, latency_ns) VALUES(?, ?, ?);";
        const stmt = try storage.Statement.prepare(self.db.conn, sql);
        defer stmt.deinit();

        stmt.bindText(1, command_tag);
        stmt.bindText(2, outcome);
        stmt.bindInt64(3, latency_ns);
        try stmt.expectDone();
    }
...
test "mac command metrics repository inserts observations" {
    ...
    try repo.insertObservation("device_time_req", "success", 42);
    try repo.insertObservation("dev_status_ans", "failure", 99);
