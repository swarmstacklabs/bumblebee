...
pub fn ingestRxpk(app: *App, allocator: std.mem.Allocator, gateway_mac: [8]u8, rxpk: Rxpk) !?IngestResult {
    ...
    const gateway = (try state_repository.loadGateway(app, allocator, gateway_hex)) orelse return null;
    defer gateway.deinit(allocator);

    const network = blk: {
        if (try state_repository.loadNetworkByName(app, allocator, gateway.network_name)) |value| break :blk value;
        return null;
    };
    defer network.deinit(allocator);

    return switch (decoded) {
        .join_request => |join| try handleJoinRequest(app, allocator, gateway_mac, gateway, network, rxpk, join),
        .data => |frame| try handleDataFrame(app, allocator, gateway_mac, gateway, network, rxpk, frame),
    };
}

fn handleJoinRequest(app: *App, allocator: std.mem.Allocator, gateway_mac: [8]u8, gateway: types.Gateway, network: types.Network, rxpk: Rxpk, join: types.JoinRequest) !?IngestResult {
    var device = (try state_repository.findDeviceByDevEui(app, allocator, join.dev_eui)) orelse return null;
    ...
    _ = try state_repository.createNodeForJoin(app, allocator, device, network, dev_addr, session.app_s_key, session.nwk_s_key);
...
fn handleDataFrame(app: *App, allocator: std.mem.Allocator, gateway_mac: [8]u8, gateway: types.Gateway, network: types.Network, rxpk: Rxpk, frame: types.DataFrame) !?IngestResult {
    var node = (try state_repository.findNodeByDevAddr(app, allocator, frame.dev_addr)) orelse return null;
    ...
    try state_repository.upsertNode(app, allocator, node);
