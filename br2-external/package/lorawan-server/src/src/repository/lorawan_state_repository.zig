...
pub fn loadGateway(app: *App, allocator: std.mem.Allocator, gateway_mac_hex: []const u8) !?types.Gateway {
    app.mutex.lock();
    defer app.mutex.unlock();

    const sql = "SELECT mac, network_name, gateway_json FROM gateways WHERE lower(mac) = lower(?);";
    const stmt = try sqlite.Statement.prepare(app.db, sql);
    defer stmt.deinit();
...
pub fn loadNetworkByName(app: *App, allocator: std.mem.Allocator, name: []const u8) !?types.Network {
    app.mutex.lock();
    defer app.mutex.unlock();
...
pub fn findDeviceByDevEui(app: *App, allocator: std.mem.Allocator, dev_eui: [8]u8) !?types.Device {
...
pub fn findNodeByDevAddr(app: *App, allocator: std.mem.Allocator, dev_addr: [4]u8) !?types.Node {
...
pub fn upsertNode(app: *App, allocator: std.mem.Allocator, node: types.Node) !void {
...
pub fn createNodeForJoin(app: *App, allocator: std.mem.Allocator, device: types.Device, network: types.Network, dev_addr: [4]u8, app_s_key: [16]u8, nwk_s_key: [16]u8) !types.Node {
    ...
    try upsertNode(app, allocator, node);
    return (try findNodeByDevAddr(app, allocator, dev_addr)).?;
}
